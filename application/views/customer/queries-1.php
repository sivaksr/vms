<?php include("header.php"); ?>


<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>
            Support
        </h1>
        <ol class="breadcrumb">
            <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Support</li>
        </ol>
    </section>

    <section class="content">
        <div class="panel with-nav-tabs panel-default">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-md-4">
                        <h3 class="mt-0 mb-0">Booking Query</h3>
                    </div>
                    <div class="col-md-8">
                        <div class="stepper">
                            <ul class="nav nav-tabs">
                                <li role="presentation" class="active">
                                    <a class="persistant-disabled" href="queries-1.php" title="Step 1">
                                        <span class="round-tab">1</span>
                                    </a>
                                </li>
                                <li role="presentation" class="disabled">
                                    <a class="persistant-disabled" href="queries-2.php" title="Step 2">
                                        <span class="round-tab">2</span>
                                    </a>
                                </li>
                                <li role="presentation" class="disabled">
                                    <a class="persistant-disabled" href="queries-3.php" title="Complete">
                                        <span class="round-tab">3</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <div class="panel-body">
                <div class="tab-content">
                    <form action="" method="">
                        <div class="row">
                            <div class="col-md-3"></div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="control-label">Question 1</label>
                                    <select class="form-control" name="">
                                        <option value="" selected disabled>Select</option>
                                        <option value="">Answer 1</option>
                                        <option value="">Answer 2</option>
                                        <option value="">Answer 3</option>
                                        <option value="">Answer 4</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Question 2</label>
                                    <select class="form-control" name="">
                                        <option value="" selected disabled>Select</option>
                                        <option value="">Answer 1</option>
                                        <option value="">Answer 2</option>
                                        <option value="">Answer 3</option>
                                        <option value="">Answer 4</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Question 3</label>
                                    <select class="form-control" name="">
                                        <option value="" selected disabled>Select</option>
                                        <option value="">Answer 1</option>
                                        <option value="">Answer 2</option>
                                        <option value="">Answer 3</option>
                                        <option value="">Answer 4</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Question 4</label>
                                    <select class="form-control" name="">
                                        <option value="" selected disabled>Select</option>
                                        <option value="">Answer 1</option>
                                        <option value="">Answer 2</option>
                                        <option value="">Answer 3</option>
                                        <option value="">Answer 4</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">Question 5</label>
                                    <select class="form-control" name="">
                                        <option value="" selected disabled>Select</option>
                                        <option value="">Answer 1</option>
                                        <option value="">Answer 2</option>
                                        <option value="">Answer 3</option>
                                        <option value="">Answer 4</option>
                                    </select>
                                </div>
                                <a href="queries-2.php" type="button" class="btn btn-primary next-step pull-right">Save and Continue</a>
                                <a href="support.php" type="button" class="btn btn-info">Back</a>
                            </div>
                            <br>
                        </div>
                        <ul class="list-inline ">
                            <li></li>
                        </ul>
                    </form>
                </div>

            </div>
        </div>
    </section>
    <!-- /.content -->
</div>
<!-- /.content-wrapper -->

<?php include("footer.php"); ?>